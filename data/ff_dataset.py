"""
FaceForensics++ Dataset implementation.

Loads data from the FF++ dataset structure using train.json, val.json, test.json
which specify the original (real) video paths. Fake videos are loaded from the
manipulated_sequences folder.
"""
...
class FFDataset(Dataset):
    ...
    This dataset loads face crops from videos in the FF++ dataset.
    Real videos are from original_sequences, fake videos from manipulated_sequences.
    ...
    def __init__(
        ...
        if not TORCH_AVAILABLE:
            raise ImportError("PyTorch is required for FFDataset")
...
class FFVideoDataset(Dataset):
    """
    FaceForensics++ Dataset that returns video-level samples.
    ...
    def __init__(
        ...
        """
        Initialize FF++ video-level dataset.
        ...
        """
        if not TORCH_AVAILABLE:
            raise ImportError("PyTorch is required for FFVideoDataset")
...
) -> Union[FFDataset, FFVideoDataset]:
    """
    Factory function to create FF++ dataset.
