#!/usr/bin/env python3
"""
Train DINO SVD Model for Deepfake Detection.

This script trains a DINO-based model with SVD residual learning where:
- Principal SVD components are frozen (preserves pre-trained knowledge)
- Residual SVD components are trainable (learns to detect forgeries)

Usage:
    # Train on single dataset
    python train_dino_svd.py --dataset ff
    python train_dino_svd.py --dataset celeb_df
    
    # Train on combined datasets
    python train_dino_svd.py --dataset combined
    
    # With custom SVD rank
    python train_dino_svd.py --dataset combined --svd_rank 767
    
    # With additional loss terms
    python train_dino_svd.py --dataset combined --use_orthogonal_loss --use_keepsv_loss
"""
...
from models.dino_svd_model import DinoSVDModel, SVDResidualLinear
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train DINO SVD Model for Deepfake Detection"
    )
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="checkpoints/dino_svd",
        help="Directory for saving checkpoints"
    )
    parser.add_argument(
        "--log_dir",
        type=str,
        default="logs/dino_svd",
...
class DinoSVDTrainer:
    """
    Trainer for DINO SVD Model.
    ...
    def _create_optimizer(self, model: DinoSVDModel) -> optim.Optimizer:
        """Create optimizer with separate parameter groups."""
        optimizer_name = self.training_config.optimizer.lower()
        lr = self.training_config.learning_rate
        weight_decay = self.training_config.weight_decay
        
        # SVD learning rate (default to main lr)
        svd_lr = self.svd_lr if self.svd_lr is not None else lr
        
        # Separate parameter groups
        svd_params = model.get_svd_params()
        classifier_params = model.get_classifier_params()
        
        parameters = [
            {"params": svd_params, "lr": svd_lr, "weight_decay": weight_decay},
    ...
    def _compute_auxiliary_losses(self, model: DinoSVDModel) -> Dict[str, torch.Tensor]:
        """Compute auxiliary losses."""
        losses = {}
        
        if self.use_orthogonal_loss:
            losses['orthogonal'] = model.compute_orthogonal_loss() * self.orthogonal_loss_weight
        
        if self.use_keepsv_loss:
            losses['keepsv'] = model.compute_keepsv_loss() * self.keepsv_loss_weight
        
        if self.use_weight_loss:
            losses['weight'] = model.compute_weight_loss() * self.weight_loss_weight
    ...
    def _save_checkpoint(self, model: nn.Module, optimizer: optim.Optimizer, 
        ...
        if filename is None:
            filename = 'checkpoint_latest_dino_svd.pt'
        
        torch.save(checkpoint, self.checkpoint_dir / filename)
        
        # Save best checkpoint
        if is_best:
            torch.save(checkpoint, self.checkpoint_dir / 'checkpoint_best_dino_svd.pt')
    ...
    def train(
        ...
        best_checkpoint = torch.load(
            self.checkpoint_dir / 'checkpoint_best_dino_svd.pt',
...
def main():
    ...
    best_checkpoint = torch.load(
            trainer.checkpoint_dir / 'checkpoint_best_dino_svd.pt',
